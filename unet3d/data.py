...
def write_image_data_to_file(image_files, data_storage, truth_storage, image_shape, n_channels, affine_storage,
                             truth_dtype=np.uint8, crop=True, label_indices=None):
    for set_of_files in image_files:
        ...
        images = reslice_image_set(set_of_files, image_shape, label_indices=label_indices, crop=crop)
...
def write_data_to_file(training_data_files, out_file, image_shape,truth_dtype=np.uint8, subject_ids=None, modality=None,
                       normalize=True, crop=True, label_indices=None):
    """
    ...
    write_image_data_to_file(training_data_files, data_storage, truth_storage, image_shape,
                             truth_dtype=truth_dtype, n_channels=n_channels, affine_storage=affine_storage, crop=crop, label_indices=label_indices)
    if subject_ids:
        hdf5_file.create_array(hdf5_file.root, 'subject_ids', obj=subject_ids)
    if normalize:
        normalize_data_storage(data_storage)
    if modality:
        hdf5_file.create_array(hdf5_file.root,'modality', obj=modality)
    hdf5_file.close()
    return out_file
